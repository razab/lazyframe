const Lazyframe = () => {
  ...
  const constants = {
    ...
    condition: {
      ...
      "youtube-playlist": (m) => (m && m[1].length == 34) ? m[1] : false,
    ...
    src: {
      ...
      "youtube-playlist": (s) => `https://www.youtube.com/embed/videoseries?list=${s.id}&${s.parameters}`,
    ...
    endpoints: {
      ...
      "youtube-playlist": (s) => `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=1&playlistId=${s.id}&key=${s.apikey}`,
    ...
    response: {
      ...
      "youtube-playlist": {
        title: (r) => r.items['0'].snippet.title,
        totalResults: (r) => r.pageInfo.totalResults,//1 per page !
        playlistId: (r) => r.items['0'].snippet.playlistId,
        videoId: (r) => r.items['0'].snippet.resourceId.videoId,
        thumbnail: (r) => {
          console.dir(r);
          let thumbs = r.items["0"].snippet.thumbnails;
          let thumb = thumbs.maxres || thumbs.standard || thumbs.high || thumbs.medium || thumbs.default;
